define([
    ...
    window.MovingFromListing = Class.create(Action, {
        ...
        moveToListingGrid: function () {
            let self = this;
            let ignoreListings = JSON.parse(Otto.customData.ignoreListings);
            $.ajax(
                    {
                        url: Otto.url.get('moveToListingGridHtml'),
                        type: 'GET',
                        data: {
                            ignoreListing: ignoreListings[0]
                        },
                        success: function (response) {
                            self.completeProgressBar();
                            self.openPopUp(response, $t('Moving Otto Items'));
                        },
                    }
            );
        },
        ...
        submit: function (listingId, onSuccess) {
            ...
            $.ajax(
                    {
                        url: Otto.url.get('moveToListing'),
                        type: 'POST',
                        data: {
                            listingId: listingId
                        },
