define([
    ...
    window.ListingOtherAutoMapping = Class.create(Action, {
        ...
        mapProductsAuto: function (product_ids) {
            ...
            this.progressBarObj.show(Otto.translator.translate('automap_progress_title'));
        ...
        sendPartsOfProducts: function (parts, totalPartsCount, isFailed) {
            ...
            if (parts.length == 0) {
                ...
                if (isFailed == 1) {
                    MessagesObj.addError(Otto.translator.translate('failed_mapped'));
                } else {
                    MessagesObj.addSuccess(Otto.translator.translate('Product was Linked.'));
                }

                this.progressBarObj.setStatus(Otto.translator.translate('task_completed_message'));
            ...
            this.progressBarObj.setStatus(str_replace('%product_title%', partExecuteString, Otto.translator.translate('processing_data_message')));

            new Ajax.Request(Otto.url.get('mapAutoToProduct'), {
                method: 'post',
                parameters: {
