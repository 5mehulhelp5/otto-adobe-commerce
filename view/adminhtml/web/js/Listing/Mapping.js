define([
    ...
    window.ListingMapping = Class.create(Common, {
        ...
        openPopUp: function (otherProductId, productTitle) {
            ...
            let self = this;
            let title = Otto.translator.translate('Linking Product');

            if (productTitle) {
                title = title + ' "' + productTitle + '"';
            }

            new Ajax.Request(Otto.url.get('mapProductPopupHtml'), {
                method: 'post',
        ...
        map: function (productId) {
            ...
            if (!confirm(Otto.translator.translate('Are you sure?'))) {
                return;
            }

            new Ajax.Request(Otto.url.get('listing_other_mapping/map', {}), {
                method: 'post',
                parameters: {
                ...
                onSuccess: function (transport) {
                    ...
                    if (response.result) {
                        ...
                        MessagesObj.addSuccess(Otto.translator.translate('Product(s) was Linked.'));
                    } else {
                        alert(Otto.translator.translate('Product does not exist.'));
                    }
                }.bind(this)
            });
        },
        ...
        remap: function (productId) {
            ...
            new Ajax.Request(Otto.url.get('listing_mapping/remap'), {
                method: 'post',
                parameters: {
                    product_id: productId,
