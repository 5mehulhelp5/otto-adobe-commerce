    ...
    window.Grid = Class.create(Common, {
        ...
        massActionSubmitClick: function () {
            ...
            $$('select#' + self.gridId + '_massaction-select option').each(function (o) {
                if (o.selected && o.value == '') {
                    self.alert(Otto.translator.translate('Please select Action.'));
                    selectAction = false;
                    return;
                }
            });
        ...
        validateItemsForMassAction: function () {
            if (this.getSelectedProductsString() === '' || this.getSelectedProductsArray().length === 0) {
                this.alert(Otto.translator.translate('Please select Items.'));
        ...
        createHelpTitleHtml: function (rowId) {
            ...
            var closeHtml = '<a href="javascript:void(0);" id="hide_item_help_' + rowId + '" title="' + Otto.translator.translate('Close') + '"><span class="hl_close icon-close"></span></a>';
            return '<div class="hl_header"><span class="hl_title">' + productTitle + '</span>' + closeHtml + '</div>';
        },

        createSynchNoteHtml: function (synchNote) {
            return '<div class="messages Otto-messages a-left">' +
        ...
        createHelpActionHtml: function (action) {
            ...
            if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_SUCCESS')) {
                classContainer += ' hl_container_success';
            } else if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_WARNING')) {
                classContainer += ' hl_container_warning';
            } else if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_ERROR')) {
                classContainer += ' hl_container_error';
            } else { // M2E_Otto_Model_Log_AbstractModel::TYPE_INFO
            ...
            if (action.action_in_progress) {
                html += '<span style="color: gray"> (' + Otto.translator.translate('In Progress') + ')</span>';
            }
            ...
            for (var i = 0; i < action.items.length; i++) {
                ...
                if (action.items[i].type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_SUCCESS')) {
                    if (action.items[i].count) {
                        type = '<span style="color: green;"> ' + action.items[i].count + ' ' +
                                Otto.translator.translate('Product(s)') +
                                '</span>';
                    } else {
                        type = '<span style="color: green;">' +
                                Otto.translator.translate('Success') +
                                '</span>';
                    }
                } else if (action.items[i].type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_WARNING')) {
                    if (action.items[i].count) {
                        ...
                        type = '<span style="color: orange;">' +
                                Otto.translator.translate('Warning') +
                                '</span>';
                    }
                } else if (action.items[i].type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_ERROR')) {
                    if (action.items[i].count) {
                        ...
                        type = '<span style="color: red;">' +
                                Otto.translator.translate('Error') +
                                '</span>';
                    }
                } else { // M2E_Otto_Model_Log_AbstractModel::TYPE_INFO
                    if (action.items[i].count) {
                        ...
                        type = '<span style="color: #444;">' +
                                Otto.translator.translate('Info') +
                                '</span>';
                    }
                }
        ...
        createHelpViewAllLogHtml: function (rowId) {
            return '<div class="hl_footer"><a target="_blank" href="' + this.getLogViewUrl(rowId) + '">' +
                    Otto.translator.translate('View Full Product Log') +
                    '</a></div>';
        },
