define([], function () {
    window.Attribute = Class.create({
        ...
        checkAttributesSelect: function (id, value) {
            if ($(id)) {
                if (typeof Otto.formData[id] != 'undefined') {
        ...
        renderAttributesWithEmptyHiddenOption: function (id, insertTo, value, width) {
            var style = width ? ' style="width: ' + width + 'px;"' : '';
            var txt = '<select name="' + id + '" id="' + id + '" class="Otto-required-when-visible select admin__control-select"' + style + '>\n';
        ...
        renderAttributesWithEmptyOption: function (id, insertTo, value, notRequiried) {
            var classes = 'Otto-custom-attribute-can-be-created select admin__control-select';

            if (!notRequiried) {
                classes += ' Otto-required-when-visible';
            }
