define([
    ...
    window.OttoTemplateCategorySpecifics = Class.create(Common, {
        ...
        initialize: function () {
            jQuery.validator.addMethod('otto-custom-specific-attribute-id', function (value, el) {

                        var customTitleInput = el;

                        var result = true;
                        $$('.otto-dictionary-specific-attribute-id').each(function (el) {
                            if (el.value == value) {
                                result = false;
                                throw $break;
                            }
                        ...
                        $$('.otto-custom-specific-attribute-id').each(function (el) {
                        ...
                        return result;
                    }, Otto.translator.translate('Item Specifics cannot have the same Labels.')
            );
        ...
        dictionarySpecificModeChange: function (index, select) {
            ...
            let recommended = $(`${scope}_dictionary_value_otto_recommended_${index}`),
            ...
            if (select.value == Otto.php.constant('\\M2E\\Otto\\Model\\Otto\\Template\\Category::VALUE_MODE_OTTO_RECOMMENDED')) {
                recommended.show().enable();
            }
            if (select.value == Otto.php.constant('\\M2E\\Otto\\Model\\Otto\\Template\\Category::VALUE_MODE_CUSTOM_VALUE')) {
                customValueTable.show();
                customValueInputs.invoke('enable');
            }
            if (select.value == Otto.php.constant('\\M2E\\Otto\\Model\\Otto\\Template\\Category::VALUE_MODE_CUSTOM_ATTRIBUTE')) {
