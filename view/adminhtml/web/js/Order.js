define([
    ...
    Order = Class.create(Common, {
        ...
        createHelpTitleHtml: function (rowId) {
            ...
            var closeHtml = '<a href="javascript:void(0);" onclick="OrderObj.hideOrderHelp(' + rowId + ');" title="' + Otto.translator.translate('Close') + '"><span class="hl_close icon-close"></span></a>';
        ...
        createHelpActionHtml: function (action) {
            ...
            } else if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_INFO')) {
            ...
            var type = '<span style="color: green;">' + Otto.translator.translate('Success') + '</span>';
            if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_INFO')) {
                type = '<span style="color: blue;">' + Otto.translator.translate('Info') + '</span>';
            } else if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_WARNING')) {
                type = '<span style="color: orange;">' + Otto.translator.translate('Warning') + '</span>';
            } else if (action.type == Otto.php.constant('M2E_Otto_Model_Log_AbstractModel::TYPE_ERROR')) {
                type = '<span style="color: red;">' + Otto.translator.translate('Error') + '</span>';
            }
        ...
        createHelpViewAllLogHtml: function (rowId, gridId) {
            let url = Otto.url.get('otto_log_order/index', {id: rowId});

            return '<div class="hl_footer"><a target="_blank" href="' + url + '">' + Otto.translator.translate('View Full Order Log') + '</a></div>';
        },
