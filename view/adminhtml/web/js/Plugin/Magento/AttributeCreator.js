define([
    ...
    window.AttributeCreator = Class.create({
        ...
        showPopup: function (params) {
            ...
            new Ajax.Request(Otto.url.get('general/getCreateAttributeHtmlPopup'), {
                method: 'post',
                asynchronous: true,
                parameters: params,
                onSuccess: function (transport) {
                    ...
                    self.popupObj = jQuery(modalDialogMessage).modal({
                        title: Otto.translator.translate('Creation of New Magento Attribute'),
                        type: 'slide',
                        buttons: [
                            {
        ...
        create: function (attributeParams) {
            ...
            new Ajax.Request(Otto.url.get('general/createAttribute'), {
                method: 'post',
                asynchronous: true,
        ...
        defaultOnSuccessCallback: function (attributeParams, result) {
            MessagesObj.addSuccess(Otto.translator.translate('Attribute has been created.'));
        ...
        chooseNewlyCreatedAttribute: function (attributeParams, result) {
            ...
            var existedOptionsCollection = self.haveOptgroup() ? $$('select[id="' + self.selectObj.id + '"] optgroup.Otto-custom-attribute-optgroup option')
        ...
        getNewlyCreatedAttributeValue: function (attributeParams) {
            ...
            var optGroupObj = $$('select[id="' + this.selectObj.id + '"] optgroup.Otto-custom-attribute-optgroup').first();

            if (optGroupObj.hasAttribute('new_option_value')) {
                return optGroupObj.getAttribute('new_option_value');
            }

            return $$('select[id="' + this.selectObj.id + '"] optgroup.Otto-custom-attribute-optgroup option').first().value;
        },
        ...
        injectAddOption: function () {
            ...
            }).update(Otto.translator.translate('Create a New One...'));

            if (self.haveOptgroup()) {
                $$('select[id="' + this.selectObj.id + '"] optgroup.Otto-custom-attribute-optgroup').first().appendChild(option);
            } else {
        ...
        validateAttributeCodeToBeUnique: function (value, el) {
            ...
            new Ajax.Request(Otto.url.get('general/isAttributeCodeUnique'), {
                method: 'post',
                asynchronous: false,
                parameters: {
                    code: value
        ...
        updateCode: function (label) {
            new Ajax.Request(Otto.url.get('general/generateAttributeCodeByLabel'), {
                method: 'post',
                asynchronous: true,
                parameters: {
                    store_label: label
        ...
        haveOptgroup: function () {
            var obj = $$('select[id="' + this.selectObj.id + '"] optgroup.Otto-custom-attribute-optgroup').first();
