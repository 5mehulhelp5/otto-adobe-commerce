        ...
        function (jQuery, confirm, localStorage) {
            window.TemplateEdit = Class.create(Common, {
                ...
                confirm: function (templateNick, confirmText, okCallback) {
                    var self = this;
                    var skipConfirmation = localStorage.get('otto_template_' + templateNick + self.skipSaveConfirmationPostFix);
                    ...
                    confirm({
                        title: Otto.translator.translate('Save Policy'),
                        content: confirmText + '<div class="admin__field admin__field-option" style="position: absolute; bottom: 43px; left: 28px;">' +
                                '<input class="admin__control-checkbox" type="checkbox" id="do_not_show_again" name="do_not_show_again">&nbsp;' + '<label for="do_not_show_again" class="admin__field-label"><span>' + Otto.translator.translate('Do not show any more') + '</span></label>' + '</div>',
                        buttons: [{
                        ...
                        actions: {
                            confirm: function () {
                                if ($('do_not_show_again').checked) {
                                    localStorage.set('otto_template_' + templateNick + self.skipSaveConfirmationPostFix, 1);
                                }
