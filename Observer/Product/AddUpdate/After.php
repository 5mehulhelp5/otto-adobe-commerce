...
class After extends AbstractAddUpdate
{
    ...
    public function beforeProcess(): void
    {
        ...
        if ($this->getProductId() <= 0) {
            throw new \M2E\Otto\Model\Exception\Logic('Product ID should be defined for "after save" event.');
        }
    ...
    private function performStatusChanges()
    {
        ...
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            $this->logListingProductMessage(
                $listingProduct,
                \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_PRODUCT_STATUS,
                $oldValue,
                $newValue
            );
        }
    ...
    private function performPriceChanges()
    {
        ...
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            $this->logListingProductMessage(
                $listingProduct,
                \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_PRODUCT_PRICE,
                $oldValue,
                $newValue
            );
        }
    ...
    private function performSpecialPriceChanges()
    {
        ...
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            $this->logListingProductMessage(
                $listingProduct,
                \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_PRODUCT_SPECIAL_PRICE,
                $oldValue,
                $newValue
            );
        }
    ...
    private function performSpecialPriceFromDateChanges()
    {
        ...
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            $this->logListingProductMessage(
                $listingProduct,
                \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_PRODUCT_SPECIAL_PRICE_FROM_DATE,
                $oldValue,
                $newValue
            );
        }
    ...
    private function performSpecialPriceToDateChanges()
    {
        ...
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            $this->logListingProductMessage(
                $listingProduct,
                \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_PRODUCT_SPECIAL_PRICE_TO_DATE,
                $oldValue,
                $newValue
            );
        }
    ...
    private function performTrackingAttributesChanges()
    {
        foreach ($this->getProxy()->getAttributes() as $attributeCode => $attributeValue) {
            ...
            foreach ($this->getAffectedListingsProductsByTrackingAttribute($attributeCode) as $listingProduct) {
                ...
                $this->logListingProductMessage(
                    $listingProduct,
                    \M2E\Otto\Model\Listing\Log::ACTION_CHANGE_CUSTOM_ATTRIBUTE,
                    $oldValue,
                    $newValue,
    ...
    private function addListingProductInstructions()
    {
        foreach ($this->getAffectedListingsProducts() as $listingProduct) {
            ...
            if ($changedAttributes = $this->listingsProductsChangedAttributes[$listingProduct->getId()] ?? null) {
    ...
    private function isProxyExist()
    {
        ...
        if (isset(\M2E\Otto\Observer\Product\AddUpdate\Before::$proxyStorage[$key])) {
            return true;
        }
        ...
        $key = $this
            ->getEvent()
            ->getProduct()
            ->getData(\M2E\Otto\Observer\Product\AddUpdate\Before::BEFORE_EVENT_KEY);

        return isset(\M2E\Otto\Observer\Product\AddUpdate\Before::$proxyStorage[$key]);
    }

    /**
     * @return \M2E\Otto\Observer\Product\AddUpdate\Before\Proxy
    ...
    private function getProxy()
    {
        ...
        $key = $this->getProductId() . '_' . $this->getStoreId();
        if (isset(\M2E\Otto\Observer\Product\AddUpdate\Before::$proxyStorage[$key])) {
            return \M2E\Otto\Observer\Product\AddUpdate\Before::$proxyStorage[$key];
        }
