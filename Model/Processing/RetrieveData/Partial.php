...
class Partial
{
    ...
    private function processRecord(\M2E\Otto\Model\Processing $processing): void
    {
        for ($i = 0; $i < self::MAX_PARTS_ON_RUN; $i++) {
            $currentPartNumber = $processing->getDataNextPart() ?? 1;

            $command = new \M2E\Otto\Model\Processing\Connector\PartialGetResultCommand(
            ...
            if ($resultCollection->has($processing->getServerHash())) {
                ...
                $result = \M2E\Otto\Model\Processing\Connector\Result::createNotFound(
                    $processing->getServerHash(),
                );
            }
