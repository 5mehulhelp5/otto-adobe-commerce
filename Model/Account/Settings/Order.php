...
class Order
{
    ...
    private array $listingOther = [
        ...
        'product_tax_class_id' => \M2E\Otto\Model\Magento\Product::TAX_CLASS_ID_NONE,
        'store_id' => \Magento\Store\Model\Store::DEFAULT_STORE_ID,
    ];
    ...
    {
        return $this->getCustomerBillingAddressMode() ===
    ...
    private function prepareShippingInformationData(array $shippingInformation): array
    {
        ...
        if (isset($shippingInformation['shipping_address_region_override'])) {
            $shippingInformation['shipping_address_region_override'] =
                (bool)(int)$shippingInformation['shipping_address_region_override'];
        }
        ...
        if (isset($customer['notifications'])) {
            if (
                ...
                if (isset($customer['notifications']['invoice_created'])) {
                    $customer['notifications']['invoice_created'] =
                        (bool)(int)$customer['notifications']['invoice_created'];
                }
                if (isset($customer['notifications']['order_created'])) {
                    $customer['notifications']['order_created'] =
                        (bool)(int)$customer['notifications']['order_created'];
                }
            } else {
                $requiredValues = [
                    'invoice_created',
