...
namespace M2E\Otto\Model\Listing\Log;

class Repository
{
    private \M2E\Otto\Model\ResourceModel\Listing\Log $resource;
    private \M2E\Otto\Model\ResourceModel\Listing\Log\CollectionFactory $listingLogCollectionFactory;

    public function __construct(
        \M2E\Otto\Model\ResourceModel\Listing\Log $resource,
    ...
    public function create(\M2E\Otto\Model\Listing\Log $log): void
    {
        $log->save();
    }
    ...
    public function removeForListing(int $listingId): void
    {
        $this->resource
            ...
            ->delete(
                $this->resource->getMainTable(),
                [sprintf('`%s` = ?', \M2E\Otto\Model\ResourceModel\Listing\Log::COLUMN_LISTING_ID) => $listingId],
            );
    }
    ...
    public function getCountErrorsByDateRange(
        ...
        $select->where('main_table.type = ?', \M2E\Otto\Model\Log\AbstractModel::TYPE_ERROR);
