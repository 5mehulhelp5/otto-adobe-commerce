...
use M2E\Otto\Model\Magento\Product\Cache;
...
class PriceCalculator
{
    ...
    private array $sourceModeMapping = [
        self::MODE_NONE => \M2E\Otto\Model\Template\SellingFormat::PRICE_MODE_NONE,
        self::MODE_PRODUCT => \M2E\Otto\Model\Template\SellingFormat::PRICE_MODE_PRODUCT,
        self::MODE_SPECIAL => \M2E\Otto\Model\Template\SellingFormat::PRICE_MODE_SPECIAL,
        self::MODE_ATTRIBUTE => \M2E\Otto\Model\Template\SellingFormat::PRICE_MODE_ATTRIBUTE,
        self::MODE_TIER => \M2E\Otto\Model\Template\SellingFormat::PRICE_MODE_TIER,
    ];

    private \M2E\Otto\Model\ProductInterface $product;

    /** @var null|string */
    private $coefficient = null;
    /** @var array */
    private $modifier = [];
    /** @var \M2E\Otto\Model\Magento\Product\Cache|null */
    ...
    public function __construct(
        \M2E\Otto\Model\ProductInterface $product,
    ...
    private function getSourceMode()
    {
        if (!in_array($this->getSource('mode'), $this->sourceModeMapping)) {
            throw new \M2E\Otto\Model\Exception\Logic('Unknown source mode.');
        }
    ...
    /**
     * @param \M2E\Otto\Model\Magento\Product\Cache|null $attributeSourceProduct
     *
     * @return \M2E\Otto\Model\Product\PriceCalculator
     */
    ...
    /**
     * @return \M2E\Otto\Model\Magento\Product\Cache|null
     */
    ...
    private function getProductBaseValue()
    {
        ...
        if ($this->isSourceModeProduct()) {
            if ($this->getMagentoProduct()->isConfigurableType()) {
                ...
                $value = $this->convertValue($this->getMagentoProduct()->getPrice());
            }
            ...
            if ($this->getMagentoProduct()->isConfigurableType()) {
                if (
                    ...
                    $value = $this->attributeHelper->convertAttributeTypePrice(
    ...
    private function getExistedProductSpecialValue(Product $product): float
    {
        ...
        if ($value <= 0) {
            return $this->convertValue($product->getPrice());
        }
    ...
    private function getExistedProductTierValue(Product $product)
    {
        ...
        foreach ($tierPrice as $qty => $value) {
            $tierPrice[$qty] = $this->convertValue($value);
        }
    ...
    /**
     * @param \M2E\Otto\Model\Magento\Product $product
     *
     * @return double
    ...
    private function getGroupedTierValue(\M2E\Otto\Model\Magento\Product $product)
    {
    ...
    private function getBundleTierFixedValue(\M2E\Otto\Model\Magento\Product $product)
    {
        return $this->calculateBundleTierValue($product, $product->getPrice());
    }

    private function getBundleTierDynamicValue(\M2E\Otto\Model\Magento\Product $product)
    {
