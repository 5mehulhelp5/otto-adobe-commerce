...
use M2E\Otto\Model\Order\Exception\ProductCreationDisabled;
...
class Reserve
{
    ...
    public const MAGENTO_RESERVATION_PLACED_EVENT_TYPE = 'otto_reservation_placed';
    public const MAGENTO_RESERVATION_RELEASED_EVENT_TYPE = 'otto_reservation_released';
    public const MAGENTO_RESERVATION_OBJECT_TYPE = 'otto_order';
    ...
    {
        if ($this->isPlaced()) {
            throw new \M2E\Otto\Model\Exception\Logic('QTY is already reserved.');
        }
    ...
    {
        if ($this->isReleased()) {
            throw new \M2E\Otto\Model\Exception\Logic('QTY is already released.');
        }
    ...
    {
        if ($this->isCanceled()) {
            throw new \M2E\Otto\Model\Exception\Logic('QTY reserve is already canceled.');
        }
    ...
    private function getValidatedOrdersItems($action)
    {
        ...
        if ($productsExistCount === 0) {
            ...
            throw new \M2E\Otto\Model\Exception\Logic('Product(s) does not exist.');
        }
    ...
    private function performAction($action, $newState)
    {
        ...
        foreach ($this->order->getItems() as $item) {
            ...
            foreach ($products as $productId => $productData) {
                ...
                /** @var \M2E\Otto\Model\Magento\Product\StockItem $magentoStockItem */
    ...
    protected function changeProductQty(
        ...
        \M2E\Otto\Model\Magento\Product\StockItem $magentoStockItem,
        $action,
        $qty,
    ...
    protected function changeMSIProductQty(
        ...
        $reservation = $this->objectManager->get(\M2E\Otto\Model\MSI\Order\Reserve::class);
    ...
    protected function pushQtyChangeInfo($qty, $action, \M2E\Otto\Model\Magento\Product $magentoProduct)
    {
        $this->qtyChangeInfo[] = [
            'action' => $action,
            'quantity' => $qty,
     ...
     */
    private function getItemProductsByAction(\M2E\Otto\Model\Order\Item $item, $action)
    {
        switch ($action) {
    ...
    private function isMsiMode(\M2E\Otto\Model\Magento\Product $product)
    {
