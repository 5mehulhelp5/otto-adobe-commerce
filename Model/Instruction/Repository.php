...
namespace M2E\Otto\Model\Instruction;
...
class Repository
{
    ...
    public function create(\M2E\Otto\Model\Instruction $instruction): void
    {
        $instruction->save();
    }
     ...
     *
     * @return \M2E\Otto\Model\Instruction[]
     */
     ...
     * @throws \Exception
     */
    public function findListingProductIdsByPriority(int $limit, ?\DateTime $excludeUntil): array
    {
        $collection = $this->collectionFactory->create();
        ...
        $collection->getSelect()->where(sprintf('pl.%s IS NULL', \M2E\Otto\Model\ResourceModel\Product\Lock::COLUMN_ID));
    ...
    public function removeOld(\DateTime $borderDate): void
    {
        $this->resource
            ...
            ->delete(
                $this->resource->getMainTable(),
                ['create_date < ?' => $borderDate->format('Y-m-d')],
            );
    }
    ...
    public function removeWithoutListingProduct(): void
    {
        ...
        $collection->getSelect()->joinLeft(
            ...
            sprintf('main_table.%s = second_table.id', InstructionResource::COLUMN_LISTING_PRODUCT_ID),
        );
