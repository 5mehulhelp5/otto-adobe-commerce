...
namespace M2E\Otto\Block\Adminhtml\Magento\Form\Element;

/**
 * Class \M2E\Otto\Block\Adminhtml\Magento\Form\Element\Select
 */
class Select extends \Magento\Framework\Data\Form\Element\Select
{
    ...
    public function getElementHtml()
    {
        ...
        if ($this->getData('create_magento_attribute') === true) {
            $this->addClass('Otto-custom-attribute-can-be-created');
        }
    ...
    protected function addCustomOptGroupAttributes(array $attributeData)
    {
        if (
            ...
            if (!isset($attributeData['attrs']) || !is_array($attributeData['attrs'])) {
                $attributeData['attrs'] = ['class' => 'Otto-custom-attribute-optgroup'];
            } else {
                if (isset($attributeData['attrs']['class'])) {
                    $attributeData['attrs']['class'] = $attributeData['attrs']['class']
                        . ' Otto-custom-attribute-optgroup';
                } else {
                    $attributeData['attrs']['class'] = 'Otto-custom-attribute-optgroup';
                }
            }
