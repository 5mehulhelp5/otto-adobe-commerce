...
namespace M2E\Otto\Block\Adminhtml\Log\Grid;

use M2E\Otto\Block\Adminhtml\Magento\AbstractBlock;
use M2E\Otto\Model\Log\AbstractModel as LogModel;
...
abstract class LastActions extends AbstractBlock
{
    ...
    {
        return base64_encode(\M2E\Otto\Helper\Json::encode($this->rows));
    }

    public function getEntityId()
    {
        if (!$this->hasData('entity_id') || !is_int($this->getData('entity_id'))) {
            throw new \M2E\Otto\Model\Exception\Logic('Entity ID is not set.');
        }
    ...
    public function getViewHelpHandler()
    {
        if (!$this->hasData('view_help_handler') || !is_string($this->getData('view_help_handler'))) {
            throw new \M2E\Otto\Model\Exception\Logic('View help handler is not set.');
        }
    ...
    public function getCloseHelpHandler()
    {
        if (!$this->hasData('hide_help_handler') || !is_string($this->getData('hide_help_handler'))) {
            throw new \M2E\Otto\Model\Exception\Logic('Close help handler is not set.');
        }
    ...
    protected function getRows()
    {
        if (!$this->hasData('logs') || !is_array($this->getData('logs'))) {
            throw new \M2E\Otto\Model\Exception\Logic('Logs are not set.');
        }
    ...
    protected function getAvailableActions()
    {
        if (!$this->hasData('available_actions') || !is_array($this->getData('available_actions'))) {
            throw new \M2E\Otto\Model\Exception\Logic('Available actions are not set.');
        }
    ...
    protected function _beforeToHtml()
    {
        ...
        $this->tip = \M2E\Otto\Helper\Data::escapeHtml($tip);
        $this->iconSrc = $this->getViewFileUrl('M2E_Otto::images/log_statuses/' . $icon . '.png');
        $this->rows = $rows;
        // ---------------------------------------
