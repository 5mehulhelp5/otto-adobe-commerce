...
namespace M2E\Otto\Block\Adminhtml\ControlPanel;

use M2E\Otto\Block\Adminhtml\Magento\Tabs\AbstractHorizontalTabs;
use M2E\Otto\Helper\View\ControlPanel as HelperControlPanel;
    ...
    protected function _prepareLayout()
    {
        ...
        $this->addTab(HelperControlPanel::TAB_OVERVIEW, [
            ...
            'content' => $this->getLayout()
                              ->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\Overview::class)
                              ->toHtml(),
        ]);
        // ---------------------------------------
        ...
        if ($activeTab === HelperControlPanel::TAB_INSPECTION) {
            $params['content'] = $this->getLayout()
                                      ->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\Inspection::class)
                                      ->toHtml();
        } else {
            $params['class'] = 'ajax';
        ...
        if ($activeTab === HelperControlPanel::TAB_DATABASE) {
            $params['content'] = $this->getLayout()
                                      ->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\Database::class)
                                      ->toHtml();
        } else {
            $params['class'] = 'ajax';
        ...
        $this->addTab(HelperControlPanel::TAB_TOOLS_MODULE, [
            ...
            'content' => $this->getLayout()
                              ->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\ToolsModule::class)
                              ->toHtml(),
        ]);

        $this->addTab(HelperControlPanel::TAB_CRON, [
            'label' => __('Cron'),
            'content' => $this->getLayout()->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\Cron::class)
                              ->toHtml(),
        ]);

        $this->addTab(HelperControlPanel::TAB_DEBUG, [
            'label' => __('Debug'),
            'content' => $this->getLayout()->createBlock(\M2E\Otto\Block\Adminhtml\ControlPanel\Tabs\Debug::class)
                              ->toHtml(),
        ]);
