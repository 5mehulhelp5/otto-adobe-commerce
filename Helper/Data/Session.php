...
class Session
{
    ...
    public function getValue($key, $clear = false)
    {
        return $this->session->getData(
            \M2E\Otto\Helper\Data::CUSTOM_IDENTIFIER . '_' . $key,
            $clear
        );
    }
    ...
    {
        $this->session->setData(\M2E\Otto\Helper\Data::CUSTOM_IDENTIFIER . '_' . $key, $value);
    }
    ...
    public function getAllValues(): array
    {
        ...
        foreach ($session as $key => $value) {
            if (strpos($key, \M2E\Otto\Helper\Data::CUSTOM_IDENTIFIER) === 0) {
                $tempReturnedKey = substr($key, strlen(\M2E\Otto\Helper\Data::CUSTOM_IDENTIFIER) + 1);
    ...
    {
        $this->session->getData(\M2E\Otto\Helper\Data::CUSTOM_IDENTIFIER . '_' . $key, true);
    }
