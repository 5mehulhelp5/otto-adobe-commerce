...
class Structure
{
    ...
    {
        return strpos($tableName, 'otto_') !== false;
    }
    ...
    public function isTableStatusOk(string $tableName)
    {
        ...
        if (!$this->isTableExists($tableName)) {
            throw new \M2E\Otto\Model\Exception("Table '$tableName' is not exists.");
        }
     ...
     * @throws \ReflectionException
     */
    public function getTableModel(string $tableName): ?string
    {
    ...
    protected function getTablesModels()
    {
        ...
        foreach ($directoryReader->readRecursively() as $directoryItem) {
            ...
            $className = '\M2E\Otto\Model\ResourceModel\\' . $modelName;

            $reflectionClass = new \ReflectionClass($className);
            if (
                $reflectionClass->isAbstract()
                || !$reflectionClass->isSubclassOf(
                    \M2E\Otto\Model\ResourceModel\ActiveRecord\AbstractModel::class
                )
            ) {
                continue;
            }

            /** @var \M2E\Otto\Model\ResourceModel\ActiveRecord\AbstractModel $object */
