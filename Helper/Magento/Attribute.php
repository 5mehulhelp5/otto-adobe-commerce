...
class Attribute extends AbstractHelper
{
    ...
    private \Magento\Catalog\Model\ResourceModel\Product\Attribute\CollectionFactory $attributeColFactory;

    private \Magento\Eav\Model\ResourceModel\Entity\Attribute\CollectionFactory $eavEntityAttributeColFactory;
    ...
    private function _getGeneralFromAttributeSets(array $attributeSetIds)
    {
        if (count($attributeSetIds) > 50) {
            throw new \M2E\Otto\Model\Exception("Attribute sets must be less then 50");
        }
    ...
    public function getSetsFromProductsWhichLacksAttributes(array $attributes, array $productIds)
    {
        ...
        $attributeSets = $this->magentoAttributeSetHelper
            ->getFromProducts(
                $productIds,
                \M2E\Otto\Helper\Magento\AbstractHelper::RETURN_TYPE_IDS
            );
